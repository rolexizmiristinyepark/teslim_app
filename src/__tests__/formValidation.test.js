...
describe('formValidation', () => {
  describe('validateSerial', () => {
    test('should return true for cufflinks (accessories) regardless of serial', () => {
      ...
      expect(validateSerial('', BrandTypes.ROLEX, CategoryTypes.AKSESUAR)).toBe(true);
      expect(validateSerial('123', BrandTypes.ROLEX, CategoryTypes.AKSESUAR)).toBe(true);
      expect(validateSerial(null, BrandTypes.ROLEX, CategoryTypes.AKSESUAR)).toBe(true);
    });

    test('should validate Rolex serial numbers correctly', () => {
      // Valid Rolex serials (8+ characters, mix of letters and numbers)
      expect(validateSerial('A1234567', BrandTypes.ROLEX, CategoryTypes.SAAT)).toBe(true);
      expect(validateSerial('AB123456', BrandTypes.ROLEX, CategoryTypes.SAAT)).toBe(true);
      expect(validateSerial('12345ABC', BrandTypes.ROLEX, CategoryTypes.SAAT)).toBe(true);
      expect(validateSerial('A1B2C3D4', BrandTypes.ROLEX, CategoryTypes.SAAT)).toBe(true);

      // Invalid Rolex serials
      expect(validateSerial('A123456', BrandTypes.ROLEX, CategoryTypes.SAAT)).toBe(false); // too short (7 chars)
      expect(validateSerial('12345678', BrandTypes.ROLEX, CategoryTypes.SAAT)).toBe(false); // no letters
      expect(validateSerial('ABCDEFGH', BrandTypes.ROLEX, CategoryTypes.SAAT)).toBe(false); // no numbers
      expect(validateSerial('', BrandTypes.ROLEX, CategoryTypes.SAAT)).toBe(false); // empty
      expect(validateSerial('   ', BrandTypes.ROLEX, CategoryTypes.SAAT)).toBe(false); // whitespace only
    });

    test('should validate Tudor serial numbers correctly', () => {
      // Valid Tudor serials (7+ characters, mix of letters and numbers)
      expect(validateSerial('A123456', BrandTypes.TUDOR, CategoryTypes.SAAT)).toBe(true);
      expect(validateSerial('AB12345', BrandTypes.TUDOR, CategoryTypes.SAAT)).toBe(true);
      expect(validateSerial('1234ABC', BrandTypes.TUDOR, CategoryTypes.SAAT)).toBe(true);
      expect(validateSerial('A1B2C3D', BrandTypes.TUDOR, CategoryTypes.SAAT)).toBe(true);

      // Invalid Tudor serials
      expect(validateSerial('A12345', BrandTypes.TUDOR, CategoryTypes.SAAT)).toBe(false); // too short (6 chars)
      expect(validateSerial('1234567', BrandTypes.TUDOR, CategoryTypes.SAAT)).toBe(false); // no letters
      expect(validateSerial('ABCDEFG', BrandTypes.TUDOR, CategoryTypes.SAAT)).toBe(false); // no numbers
      expect(validateSerial('', BrandTypes.TUDOR, CategoryTypes.SAAT)).toBe(false); // empty
    });
  ...
  describe('getSerialMinLength', () => {
    test('should return correct minimum lengths for each brand', () => {
      ...
      expect(getSerialMinLength(BrandTypes.ROLEX, CategoryTypes.AKSESUAR)).toBe(0); // cufflinks
    });
  ...
  describe('getSerialDescription', () => {
    test('should return correct descriptions for each brand', () => {
      expect(getSerialDescription(BrandTypes.ROLEX, CategoryTypes.SAAT))
        .toBe('En az 8 karakter, en az 1 harf ve 1 rakam içermeli');
      
      expect(getSerialDescription(BrandTypes.TUDOR, CategoryTypes.SAAT))
        .toBe('En az 7 karakter, en az 1 harf ve 1 rakam içermeli');
      
      expect(getSerialDescription(BrandTypes.ROLEX, CategoryTypes.AKSESUAR))
